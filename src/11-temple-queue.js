...
export class TempleQueue {
  ...
  constructor(templeName, maxCapacity) {
    ...
    this.#maxCapacity = maxCapacity ? maxCapacity : 100;
    this.#devotees = [];
    this.#vipEnabled = false;
  }
  ...
  contains(name) {
    // Your code here
    return this.#devotees.some((i) => i.name === name);
  }

  toArray() {
    // Your code here
    return this.#devotees.map((i) => i);
  }
  ...
  static merge(queue1, queue2) {
    ...
    for (const devotee of queue1.#devotees) {
      mergedQueue.#devotees.push(devotee);
    }

    for (const devotee of queue2.#devotees) {
      mergedQueue.#devotees.push(devotee);
    }
  ...
  [Symbol.iterator]() {
    // Your code here
    let index = 0
    const devotee = this.#devotees

    return{
      next() {
        if (index < devotee.length){
          return {value: devotee[index++], done: false}
        }return {done: true}
      }
    }
  }

...
export function assignRider(order) {
  ...
  const selectedRider = ["Rahul", "Priya", "Amit", "Neha", "Vikram"];
  return new Promise((resolve, reject) => {
    ...
    resolve({
      ...order,
      rider: selectedRider[Math.floor(Math.random() * selectedRider.length)],
      status: "assigned",
    });
  });
}
...
export function deliverOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if (order.status !== "assigned") reject(new Error("No rider assigned!"));
...
export function processMultipleOrders(orderList) {
  ...
  for (const element of orderList) {
    result.push(processDelivery(element.restaurant, element.items));
  }
  console.log(result);

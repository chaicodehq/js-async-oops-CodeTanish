...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const average = this.innings.reduce(
      (sum, curr) => sum + curr.strikeRate,
      0,
    );

    return average / this.innings.length;
  }

  getProfile() {
    // Your code here
    const totalRuns = this.innings.reduce((sum, curr) => sum + curr.runs, 0);
    return {
      ...super.getProfile(),
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalEconomy = this.spells.reduce(
      (sum, curr) => sum + curr.economy,
      0,
    );
    const length = this.spells.length;

    return totalEconomy / length;
  }

  getProfile() {
    // Your code here
    const totalWickets = this.spells.reduce(
      (sum, curr) => sum + curr.wickets,
      0,
    );
    return {
      ...super.getProfile(),
...
export class AllRounder extends Player {
  ...
  getStrikeRate() {
    ...
    const total = this.innings.reduce((sum, curr) => sum + curr.strikeRate, 0);
  ...
  getEconomy() {
    ...
    const total = this.spells.reduce((sum, curr) => sum + curr.economy, 0);
